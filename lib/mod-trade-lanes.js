...
var LANE_CONFIG = {
  day: {
    ...
    color: 'var(--mp-cyan)',
    rawColor: '#00AEFF',
    bg: 'rgba(0,174,255,0.04)',
    border: 'rgba(0,174,255,0.15)'
  },
  swing: {
    title: 'Swing Trade',
    subtitle: '3\u201314 day window',
    color: 'var(--mp-text-primary)',
    rawColor: 'var(--mp-text-primary)',
    bg: 'transparent',
...
function buildLaneCard(r, laneColor){
  ...
  var tierBadge = '';
  if(tier === 'PLATINUM') tierBadge = '<span class="tl-tier tl-platinum">Platinum</span>';
  else if(tier === 'GOLD') tierBadge = '<span class="tl-tier tl-gold">Gold</span>';
  else if(tier === 'SURPRISE') tierBadge = '<span class="tl-tier tl-surprise">Surprise</span>';
...
function buildLane(key, cards){
  ...
  var html = '<div class="tl-lane">'
    + '<div class="tl-lane-header" style="border-left:3px solid '+cfg.rawColor+';background:'+cfg.bg+';">'
      + '<div>'
  ...
  if(overflow.length){
    ...
    html += '</div>';
    html += '<button class="tl-view-more" data-target="'+laneId+'">'
