...
(function(){
  ...
  var NAV_CONFIG = [
    ...
    // Stock Prism opens the standalone /_ticker page directly — bypasses the
    // dashboard's iframe-based ticker tab (which produced a duplicate sidebar).
    // Keeps tab:'ticker' so the active-state highlight still matches when the
    // ticker page renders with activeTab:'ticker'.
    { type:'item',    tab:'ticker',      href:'/_ticker?ticker=AAPL', label:'Stock Prism', icon:ICONS.ticker, badge:{ style:BADGE_NEW_TEAL_DIM, text:'NEW' } },
    ...
    { type:'item',    href:'/search',    label:'Search Tickers', icon:ICONS.search,
                      badge:{ style:BADGE_TESTING_AMBER, text:'TESTING' } },
  ...
  function renderItem(item, mode, activeTab){
    ...
    if (item.href){
      return '<a class="'+cls+'" href="'+escAttr(item.href)+'" style="text-decoration:none;"'+titleAttr+'>'+icon+label+badge+'</a>';
    }

    if (mode === 'tabs' && item.tab){
      return '<button class="'+cls+'" data-tab="'+escAttr(item.tab)+'" onclick="switchTab(\''+item.tab+'\',this)"'+titleAttr+'>'+icon+label+badge+'</button>';
    }

    return '<a class="'+cls+'" href="'+DASHBOARD_URL+'?tab='+escAttr(item.tab)+'" style="text-decoration:none;"'+titleAttr+'>'+icon+label+badge+'</a>';
  }
