...
(function(){
  ...
  var NAV_CONFIG = [
    ...
    { type:'item',    tab:'prism',       label:'Prism',          icon:ICONS.prism,       badge:{ style:BADGE_NEW_GREEN, text:'NEW' } },
    ...
    { type:'item',    tab:'ticker',      label:'Stock Research', icon:ICONS.ticker,      badge:{ style:BADGE_NEW_TEAL_DIM, text:'NEW' } },
    { type:'item',    tab:'validation',  label:'Track Record',   icon:ICONS.validation,  badge:{ cls:'teal', text:'NEW', id:'nav-badge-validation' } },
    ...
    { type:'item',    href:'/search', label:'Search Tickers', icon:ICONS.search, badge:{ style:BADGE_TESTING_AMBER, text:'TESTING' } },
  ...
  function renderItem(item, mode, activeTab){
    ...
    if (item.href){
      return '<a class="'+cls+'" href="'+escAttr(item.href)+'" style="text-decoration:none;">'+icon+label+badge+'</a>';
    }

    if (mode === 'tabs' && item.tab){
      return '<button class="'+cls+'" data-tab="'+escAttr(item.tab)+'" onclick="switchTab(\''+item.tab+'\',this)">'+icon+label+badge+'</button>';
    }

    return '<a class="'+cls+'" href="'+DASHBOARD_URL+'?tab='+escAttr(item.tab)+'" style="text-decoration:none;">'+icon+label+badge+'</a>';
  }
  ...
  var SIDEBAR_CSS = ''+
    ...
    '.side-nav{position:fixed;top:0;left:0;width:248px;height:100vh;background:#FFFFFF;border-right:1px solid rgba(0,0,0,0.08);display:flex;flex-direction:column;z-index:100;overflow-y:auto;transform:translateX(-100%);transition:transform .25s ease;}'+
    ...
    '@media(min-width:768px){.side-nav.open ~ .main, .side-nav.open ~ .main-content{margin-left:248px;}}'+
